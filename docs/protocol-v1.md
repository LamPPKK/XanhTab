...
The release builder creates the base FST from reviewed local source files and a provenance document; it does not accept an opaque prebuilt FST. Each source record binds one safe filename to its URL, revision, SHA-256, license identifier, license URL, and whether redistribution was reviewed. The builder rejects symlinks, oversized inputs, checksum mismatches, an empty result, and any mismatch between the generated unique-key count and `entry_count`. Both builder and installer require `redistribution=reviewed`; there is no automatic fallback to the latest upstream list. Archive checksums authenticate the generated FST and metadata before APT mutation. Installation then performs a streaming union with the validated local hosts file and atomically installs the resulting read-only FST.
