...
| X2 | Responsive clientless dashboard, navigation, session policy controls, profile ladder, version metrics, purpose-bound signaling relay, and five fail-closed adapter contracts. Netd now stages a UID kill-switch before teardown/setup, pins proxy endpoints, gives WireGuard its own UID policy table, and reports committed state rather than a synthetic Direct status | End-to-end rswebrtc consumer handshake plus five-adapter public-IP/DNS/kill-switch hardware matrix |
...
- `xanhtab-netd`: minimal privileged egress helper using validated enum commands, absolute allowlisted executables, a transition kill-switch, and stateful policy reporting.
...
The installer stops before mutation on a wrong OS, architecture, board, disabled memory cgroup, or missing cgroup v2 `memory` controller. Verification tools must already be installed. It downloads and verifies the signed manifest and then enforces its exact v1 contract: no unknown fields, complete component versions, exactly one safe-basename ARM64 artifact, a credential-free HTTPS URL and a lowercase SHA-256. It verifies the archive before APT mutation, rejects unsafe archive paths and symlinks, validates every component checksum and the plugin architecture, then checks the plugin against the installed GStreamer ABI. Root-only egress secrets are decoded into the private installer workspace and checked by the signed staged netd before APT: proxy credentials are never printed, the firewall endpoint is derived from the same URL, and WireGuard must pass the strict no-hooks/full-tunnel policy. Before replacing product files the installer records service enable/running state and backs up all XanhTab-owned paths, including `/var/lib/xanhtab`. Any later failure restores those paths and service states; newly installed APT dependencies and newly allocated system accounts are deliberately outside that rollback boundary. The installer validates config, starts all services, and prints the QR only after health succeeds.

A production package must provide `BLOCKLIST_SOURCE_DIR` and `BLOCKLIST_METADATA` to `scripts/package-release.sh`; neither has an implicit network fallback. The metadata maps each safe local filename to its source URL, revision, SHA-256, license identifier, license URL, redistribution status, and the expected unique-entry count. The builder reads only those named files and rejects symlinks, oversize inputs, checksum mismatches and entry-count drift, then creates the base FST itself; it does not accept an opaque prebuilt FST from the release operator. Packaging and installation both reject a source unless redistribution is explicitly `reviewed`. The signed archive covers the resulting base FST and provenance document, and the installer validates both before APT mutation. On-device compilation performs a streaming union of that base FST and validated local `custom_hosts.txt`, so the complete upstream list is never expanded into an in-memory string set. This repository deliberately does not vendor StevenBlack or another aggregated list until its exact source composition and redistribution terms have been reviewed.
...
For Git-backed public configuration, both `--config-repo` and immutable `--config-ref` are required. Only `config.json`, `custom_hosts.txt`, `bookmarks.json`, and `blocklist-metadata.json` are copied, each with a repository checksum and a 1 MiB limit. Symlinks are rejected before any root read. The signed staged daemon validates the three versioned JSON contracts and every custom hostname before APT or product-file mutation. On a fresh install, `config.json` can merge only reviewed non-secret session defaults (`initial_url`, stream profile and auto-burn) into a new validated production TOML; it cannot change TLS, paths, UIDs, egress or secret references. Bookmarks accept credential-free HTTP(S) URLs; blocklist metadata carries bounded HTTPS provenance, lowercase source checksums, license references and an explicit redistribution status. Remote metadata is retained as non-secret configuration; it cannot replace the signed packaged base FST. Secrets never come from that repository.

`--secrets-file` accepts a regular, root-owned mode-0600 JSON file of at most 1 MiB with only `wireguard_config_base64`, `proxy_url`, `stun_config_base64`, and `turn_config_base64`. Values are not logged; ICE files remain root-owned references and are not embedded in public config. A custom proxy URL must use HTTP(S) or SOCKS5, contain a literal IP endpoint, and match the nftables allow rule derived during installation. WireGuard uses only the dedicated `wg0` interface, requires `Table=off` plus a full-tunnel peer, and rejects `DNS`, `SaveConfig`, every up/down hook, unknown sections, unsafe permissions, and symlinks before `wg-quick` can read the file. WARP local proxy traffic is encrypted egress, not anonymity; Tor uses remote DNS. A mode becomes active only after its local/remote proxy listener or WireGuard interface is ready and the final nftables policy commits. Public-IP and DNS-leak proof still belongs to the Pi hardware matrix, not these host-side checks.
